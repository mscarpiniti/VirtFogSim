% Setting all the global variables for DAG #11
%
% DAG #11 describes a CyberShake workflow with V = 25 tasks.
%
%  ***************************************************
%  * Package: VirtFogSim                             *
...
%
V = 25;  % Positive scalar integer value. It is the number of tasks of the 
...
k2 = 1;
%
s = k1*c1*[26,50,350,30*ones(1,4),130*ones(1,4),40*ones(1,4),4, 40*ones(1,4), 30*ones(1,2), 92*ones(1,2),26]; 		%DAG_A (CyberShake workflow)
%
...
%
A = [0, 1, 1, zeros(1,22);          		%DAG_A (CyberShake workflow)
     0, 0, 0, ones(1,4), zeros(1,18);
     zeros(1,7), ones(1,4), zeros(1,14);
     zeros(1,11), 1, zeros(1,3), 1, zeros(1,9);
     zeros(1,12), 1, zeros(1,2), 1, zeros(1,9);
     zeros(1,13), 1, 0, 1, zeros(1,9);
     zeros(1,14), 1, 1, zeros(1,9);
     zeros(1,15), 1, 1, zeros(1,8);
     zeros(1,18), 1, zeros(1,6);
     zeros(1,15), 1, 0, 1, zeros(1,7);
     zeros(1,15), 1, 0, 0, 1, zeros(1,6);
     zeros(1,15), 1, 0, 0, 0, 1, zeros(1,5); 
     zeros(1,20), 1, zeros(1,4);
     zeros(1,20), 1, zeros(1,4);
     zeros(1,21), 1, zeros(1,3);
     zeros(1,24), 1;
     zeros(1,22), 1, 0, 0;
     zeros(1,22), 1, 0, 0;
     zeros(1,23), 1, 0;
...
%
Da = k2*c2*[0, 10, 110, zeros(1,22);          %DAG_A (CyberShake workflow)
     0, 0, 0, 50*ones(1,4), zeros(1,18);
     zeros(1,7), 135.5*ones(1,4), zeros(1,14);
     zeros(1,11), 40, zeros(1,3), 2, zeros(1,9);
     zeros(1,12), 40, zeros(1,2), 2, zeros(1,9);
     zeros(1,13), 40, 0, 2, zeros(1,9);
     zeros(1,14), 40, 2, zeros(1,9);
     zeros(1,15), 2, 40, zeros(1,8);
     zeros(1,15), 2, 0, 40, zeros(1,7);
     zeros(1,15), 2, 0, 0, 40, zeros(1,6);
     zeros(1,15), 2, 0, 0, 0, 40, zeros(1,5);
     zeros(1,20), 30, zeros(1,4);
     zeros(1,20), 30, zeros(1,4);
     zeros(1,21), 30, zeros(1,3);
     zeros(1,21), 30, zeros(1,3);
     zeros(1,24), 128;
     zeros(1,22), 30, 0, 0;
     zeros(1,22), 30, 0, 0;
...
%
TH_MIN_0 = 1/0.3; %Scalar non-negative real parameter, i.e., TH_MIN_0 >= 0. 
...
%
P_IDLE_CPU_C = 440;    %Power consumed by a single physical server in the
...
%
nc_F = 16;  % Dimension-less scalar positive integer parameter, 
            % i.e., nc_F >= 1. It is the average number of containers that 
            % SIMULTANEOUSLY run atop a SINGLE physical server at the 
            % Fog node.   
%
nc_C = 24;  % Dimension-less scalar positive integer parameter, 
...
gamma_C = 3.0; % Dimension-less positive scalar exponent of the dynamic power 
             ...
             % Typically, gamma_C = 3. 
%
K_M = 7.5e-21; % Positive scalar parameter. It is the profile of the dynamic 
...
%
K_C = 3.5*3.26e-20;% Positive scalar parameter. It is the profile of the dynamic 
...
%
%
Prob_LOSS = 1.56e-5; % Average segment loss probability of the TCP/IP 
...
%
chi_TX_CELL = 6*3.85e-14;% Power profile of the cellular NIC in the 
...
%
a_max = 9.9e-8; % Dimension-less positive scalar real parameter, 
...
PS = 150;       % Dimension-less positive even integer scalar parameter. 
...
%
%
a_max_vec_FT = [1.15e-7,9.9e-8,9.3e-8];  
