% Setting all the global variables for DAG #13
%
% DAG #13 describes a Molecular Chatics Dynamic workflow with V = 41 tasks.
%
%  ***************************************************
%  * Package: VirtFogSim                             *
...
%
V = 41;  % Positive scalar integer value. It is the number of tasks of the 
...
k2 = 1;
%
s = k1*c1*[5, 132, 133, 26, 52, 53, 26, 27, 32, 33, 32, 26, 53, 52, 26, 45, ... 
           50, 42, 53, 18, 40, 27, 48, 55, 50, 28, 52, 27, 25, 28, 52, 50, ... 
           51, 53, 23, 50, 30, 48, 25, 27, 5];        %DAG_W (Molecular dynamics workflow)
...
%
A = [0, 1, 1, 1, zeros(1,37);%1             %DAG_W (Molecular dynamics workflow)
     zeros(1,4),  1, 0, 1, zeros(1,34);%2
     zeros(1,5),  1, 1, zeros(1,34);%3
     zeros(1,7),  1, 1, 1, 1, zeros(1,30);%4
     zeros(1,11), 1, zeros(1,29);%5
     zeros(1,11), 1, zeros(1,29);%6
     zeros(1,12), 1, zeros(1,14), 1, 1, zeros(1,12);%7
     zeros(1,13), 1, zeros(1,27);%8
     zeros(1,13), 1, zeros(1,27);%9
     zeros(1,14), 1, zeros(1,26);%10
     zeros(1,14), 1, zeros(1,26);%11                                                               
     zeros(1,15), 1, 1, 1, 1, 1, 1, zeros(1,20);%12
     zeros(1,15), 1, 1, 1, 1, 1, 1, zeros(1,20);%13
     zeros(1,20), 1, 1, zeros(1,19);%14
     zeros(1,21), 1, zeros(1,19);%15
     zeros(1,22), 1, 1, 1, zeros(1,16);%16
     zeros(1,25), 1, zeros(1,15);%17
     zeros(1,25), 1, zeros(1,15);%18
     zeros(1,25), 1, 1, zeros(1,14);%19
     zeros(1,26), 1, zeros(1,14);%20
     zeros(1,26), 1, zeros(1,6), 1, zeros(1,7);%21                                                           
     zeros(1,27), 1, 1, zeros(1,12);%22
     zeros(1,29), 1, zeros(1,11);%23
     zeros(1,29), 1, 1, zeros(1,10);%24
     zeros(1,30), 1, 1, zeros(1,9);%25
     zeros(1,32), 1, zeros(1,8);%26
     zeros(1,32), 1, zeros(1,8);%27
     zeros(1,31), 1, 0, 1, zeros(1,7);%28
     zeros(1,33), 1, 1, zeros(1,6);%29
     zeros(1,35), 1, zeros(1,5);%30
     zeros(1,35), 1, zeros(1,5);%31
     zeros(1,36), 1, zeros(1,4);%32
     zeros(1,36), 1, zeros(1,4);%33
     zeros(1,37), 1, zeros(1,3);%34
     zeros(1,37), 1, zeros(1,3);%35
     zeros(1,38), 1, 0, 0;%36
     zeros(1,38), 1, 1, 0;%37
     zeros(1,39), 1, 0;%38
     zeros(1,40), 1;%39
...
%
Da = k2*c2*[0, 1, 1, 24, zeros(1,37);%1      %DAG_W (Molecular dynamics workflow)
     zeros(1,4),  5, 0, 17, zeros(1,34);%2
     zeros(1,5),  19, 25, zeros(1,34);%3
     zeros(1,7),  26, 11, 24, 22, zeros(1,30);%4
     zeros(1,11), 25, zeros(1,29);%5
     zeros(1,11), 27, zeros(1,29);%6
     zeros(1,12), 24, zeros(1,14), 16, 25, zeros(1,12);%7
     zeros(1,13), 15, zeros(1,27);%8
     zeros(1,13), 26, zeros(1,27);%9
     zeros(1,14), 25, zeros(1,26);%10
     zeros(1,14), 11, zeros(1,26);%11                                                               
     zeros(1,15), 9,  24, 32, 18, 27, 13, zeros(1,20);%12
     zeros(1,15), 29, 14, 24, 32, 35, 37, zeros(1,20);%13
     zeros(1,20), 26, 9, zeros(1,19);%14
     zeros(1,21), 10, zeros(1,19);%15
     zeros(1,22), 14, 38, 14, zeros(1,16);%16
     zeros(1,25), 31, zeros(1,15);%17
     zeros(1,25), 14, zeros(1,15);%18
     zeros(1,25), 46, 18, zeros(1,14);%19
     zeros(1,26), 12, zeros(1,14);%20
     zeros(1,26), 14, zeros(1,6), 36, zeros(1,7);%21                                                           
     zeros(1,27), 29, 23, zeros(1,12);%22
     zeros(1,29), 18, zeros(1,11);%23
     zeros(1,29), 37, 27, zeros(1,10);%24
     zeros(1,30), 25, 40, zeros(1,9);%25
     zeros(1,32), 15, zeros(1,8);%26
     zeros(1,32), 29, zeros(1,8);%27
     zeros(1,31), 29, 0, 19, zeros(1,7);%28
     zeros(1,33), 26, 7, zeros(1,6);%29
     zeros(1,35),  6, zeros(1,5);%30
     zeros(1,35), 25, zeros(1,5);%31
     zeros(1,36), 35, zeros(1,4);%32
     zeros(1,36), 30, zeros(1,4);%33
     zeros(1,37),  9, zeros(1,3);%34
     zeros(1,37), 27, zeros(1,3);%35
     zeros(1,38), 22,  0, 0;%36
     zeros(1,38),  5, 18, 0;%37
     zeros(1,39), 10,  0;%38
     zeros(1,40), 95;%39
     zeros(1,40), 95;%40
     zeros(1,41)];
...
TH_MIN_0 = 1/0.3; %1/0.3; % Scalar non-negative real parameter, i.e., TH_MIN_0 >= 0. 
                % It is the minimum required application throughput and it 
...
gamma_M =  3.2;% Dimension-less positive scalar exponent of the dynamic power 
             ...
             % Typically, gamma_M = 3. 
%
gamma_F = 3.1; % Dimension-less positive scalar exponent of the dynamic power 
             % consumption of a single physical server at the Fog node. 
             % Typically, gamma_F = 3. 
%
gamma_C = 3.0;% Dimension-less positive scalar exponent of the dynamic power 
...
%
zeta_TX_WiFi = 2.4;     % Dimension-less positive scalar exponent 
                        % of the dynamic power consumption of the WiFi NIC 
                        % in the transmit mode.
%
zeta_RX_WiFi = 2.2;     % Dimension-less positive scalar exponent 
                        % of the dynamic power consumption of the WiFi NIC 
                        % in the receive mode.
%
zeta_TX_CELL = 2.45;    % Dimension-less positive scalar exponent 
                        % of the dynamic power consumption of the cellular 
                        % 3G/4G NIC in the transmit mode.
%
zeta_RX_CELL = 2.34;    % Dimension-less positive scalar exponent 
                        % of the dynamic power consumption of the cellular
...
%
a_max = 2.2e-7; % Dimension-less positive scalar real parameter, 
...
PS = 450;       % Dimension-less positive even integer scalar parameter. 
...
CF = 0.5;       % Fraction of the population size PS that is subject 
                ...
                % It must be: 0 < CF < 1.
%
G_MAX = 100; % Dimension-less positive scalar integer parameter, 
...
%
%
a_max_vec_FT = [2.23e-7,2.20e-7,2.14e-7];  
